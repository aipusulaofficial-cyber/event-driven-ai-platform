...
variable "image" { type=string default="event-driven-ai-platform:latest" }
resource "kubernetes_deployment" "api" {
  metadata { name="event-driven-ai-platform" namespace=var.namespace labels={app="event-driven-ai-platform"} }
  spec { replicas=var.replicas selector { match_labels={app="event-driven-ai-platform"} } template {
    metadata { labels={app="event-driven-ai-platform"} }
...
resource "kubernetes_service" "api" {
 metadata {name="event-driven-ai-platform" namespace=var.namespace}
 spec { selector={app="event-driven-ai-platform"} port {port=80 target_port=8000} }
}
