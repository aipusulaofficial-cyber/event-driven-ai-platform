...
## Core boundary
**Primary concern:** Event processing.

Event contracts are versioned; ingestion, domain handling, and delivery remain separated; consumers are idempotent; retry/dead-letter semantics are explicit; queue and batch growth are bounded.
